...
struct nSat {
  ...
  void feedback_algorithm() {
    ...
    if(beta > 0) {
      Amplitude = k1a * pow(dist,2) * (pow(abs(beta),0.5));
    }
    else {
      Amplitude = -1 * k1a * pow(dist,2) * (pow(abs(beta),0.5));
    }
...
/*===========================================================*/
/*
double sensordistRead(Adafruit_VL53L0X* sens, VL53L0X_RangingMeasurementData_t* meas) {
  double sum = 0;
  double final_relative_dist = 0;

  sens->rangingTest(meas, false);

  for(int kk=1; kk<5000; ++kk) {
    sum += meas->RangeMilliMeter; // Range in millimeters + offset
  }
  final_relative_dist = ((sum/1000.00)+0.210612583)/5000.00;
  return final_relative_dist;
}
*/
/*===========================================================*/
...
/*===========================================================*/
/*
void velocity_func(nSat &sat_t, Adafruit_VL53L0X* sens, VL53L0X_RangingMeasurementData_t* meas) {
  sat_t.dist_hist[sat_t.i] = sensordistRead(sens, meas);
...
void feedback_algorithm(nSat &sat_t) {
  beta = tanh(kr * (sat_t.dist - desired_dist)) + c*tanh(kv * sat_t.vel);
  if(beta > 0) {
    Amplitude = k1a * pow(sat_t.dist,2) * (pow(abs(beta),0.5));
  }
  else {
    Amplitude = -1 * k1a * pow(sat_t.dist,2) * (pow(abs(beta),0.5));
  }
...
void initLIDAR() {
  ...
  if(!sens.begin(SADDR, DBG, i2c)) {
    Serial.println("Failed to boot SAT1 VL53L0X");
    while(1);
  }
  */
}
